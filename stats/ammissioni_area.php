...
echo '
	<div class="alert alert-dismissible alert-warning">
  		<h2 class="alert-heading">Soci ammessi '.$titolofiliale.'</h2>
  		<p class="mb-0 justify-content-between align-items-left">Questo report rappresenta la situazione attuale (aggiornata al '.$adesso.') di tutte le richieste di ammissione avanzate al CDA.</p>
	</div>
';
...
// -------------------------------------------------------------------------------
// -------------------------------------------------------------------------------
$strQuery3 = "SELECT count(*) as qta, CONCAT(substring(CDA,7,4), substring(CDA,4,2))  as AnnoMeseRichiesta 
                  FROM tab_xls_ammissioni
                  WHERE filiale in ".$_GET['key']."
...
// -------------------------------------------------------------------------------
    ...
    $myfile = fopen("../tmp/ammissioni".$filiale.".csv", "w");
    $contenutofile .= "Filiale;CAG;Conto;MancaDB;Nominativo;DataDomanda;NumAzioni;SuccCess;AzioniDaSuccCess;UlterioriAzioniDaSuccCess;Note\n";
    while($cnt_file = mysqli_fetch_array($qry_file)){ 
        $contenutofile .= 
			...
			$cnt_file['Manca_DB'].";".
			$cnt_file['Nominativo'].";".
			$cnt_file['Data_Domanda'].";".						
			$cnt_file['Azioni_Sottoscritte'].";".			
			$cnt_file['Flag_da_SUCC_CESS'].";".
			$cnt_file['Qta_Azioni_da_Succ_Cess'].";".
			$cnt_file['Qta_Ulteriori_azioni_da_acquistare'].";".
			$cnt_file['Note']."\n";
    }
    fwrite($myfile, $contenutofile);
    fclose($myfile);

    echo '<br><center><a class="btn btn-outline-warning" href="../tmp/ammissioni'.$filiale.'.csv">Scarica il dettaglio completo</a>';
