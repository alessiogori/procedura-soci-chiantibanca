...
// -------------------------------------------------------------------------------
// -------------------------------------------------------------------------------
// TREND PER MESE/ANNO - IMPORTO
// -------------------------------------------------------------------------------
// -------------------------------------------------------------------------------
   
$trendimporto = "SELECT  AnnoMeseRichiesta,
...
$result_trendimporto = $dbhandle->query($trendimporto) or exit("Error code ({$dbhandle->errno}): {$dbhandle->error}");

  if ($result_trendimporto) {
        	
	$arrDataTrendimporto = array(
        "chart" => array(
        	"caption"=> "Ammissioni ed Uscite di Soci (importo)",
        	"captionFont" => "Arial",
        	"captionFontSize" => "24",
        	...
        	while($rowTrendimporto = mysqli_fetch_array($result_trendimporto)) {				
				    array_push($categoryArray, array(
					  "label" => $rowTrendimporto["AnnoMeseRichiesta"]
					)
				);

				array_push($dataseries1, array(
					"value" => $rowTrendimporto["TotaleUscite"]
					//"value" => number_format($row["Raccolta"], 0, ',', '.')
					) 
				);
			
				array_push($dataseries2, array(
					"value" => $rowTrendimporto["TotaleEntrate"]
    	...
    	$arrDataTrendimporto["categories"]=array(array("category"=>$categoryArray));

			// creating dataset object
			$arrDataTrendimporto["dataset"] = array(array("seriesName"=> "Uscite", "data"=>$dataseries1), array("seriesName"=> "Ammissioni", "data"=>$dataseries2));

      $jsonEncodedData = json_encode($arrDataTrendimporto);
      // chart object
      $msChart = new FusionCharts("msline", "chartImporto" , "100%", "400", "trendimporto", "json", $jsonEncodedData);
      $msChart->render();
...
<table border="0" align="center">
  <tr>     
       <td><div id="trendimporto"><!-- Fusion Charts will also be rendered here--></div></td>
  </tr>
</table>
<br>
...
<table border="0" align="center" width="90%">
  <tr>     
       <td valign="top" align="center" width="30%">
        <form class="form-inline my-2 my-lg-0" action="andamentale_grafico_trendimporto.php" method="GET" onsubmit="return ray.ajax()">
