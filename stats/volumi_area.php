...
$strQuery = "   SELECT codFil, round(sum(Raccolta)/1000) as Raccolta, round((sum(Impieghi)*-1)/1000) as Impieghi 
  ...
  if ($result) {
	...
	$arrData = array(
        "chart" => array(
        	"caption"=> "Raccolta e Impieghi per Area ".$filiale." (in milioni)",
        	"captionFontSize" => "24",
...
$result_areeT = $dbhandle->query($dettaglioT) or exit("Error code ({$dbhandle->errno}): {$dbhandle->error}");
  ...
  while ($row_areeT = mysqli_fetch_array($result_areeT)) {
    ...
    $tab_dettaglioA .=  "<tr class='table-secondary'>
            <td align='right'>".$row_areeA['Filiale']."</td>
            <td align='right'>".number_format($row_areeT['qta'],0,',','.')."&nbsp;</td>
            <td align='right'>".number_format($row_areeT['NumAzTot'],0,',','.')."</td>            
            <td align='right'>".number_format($row_areeT['ValNomTot'],0,',','.')."&nbsp;</td>
            <td align='right'>".number_format($row_areeT['NumCC'],0,',','.')."</td>            
            <td align='right'>".number_format($row_areeT['NumCarte'],0,',','.')."&nbsp;</td>
            <td align='right'>".number_format($row_areeT['NumTitPol'],0,',','.')."</td>
            <td align='right'>".number_format($row_areeT['NumHB'],0,',','.')."&nbsp;</td>
            <td align='right'>".number_format($row_areeT['Raccolta'],0,',','.')."</td>
            <td align='right'>".number_format($row_areeT['Impieghi'],0,',','.')."</td>
          </tr>
        ";
  }
...
<center>
 <div id="chart-container">Area - Raccolta e Impieghi</div></center>
