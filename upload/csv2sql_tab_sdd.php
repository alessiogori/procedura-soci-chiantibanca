...
$titolo  = "Caricamento SDD Mutua per Check Soci Banca";
$tabella = "tab_sdd";
$nomefile= "sdd.xml";
$istruzioni = '	1. Estrai da Gestionale Comipa il tracciato SDD per Inbank<br>
                2. Rinominalo <b>sdd.xml</a> <br>
				3. Caricalo su questa pagina (sfoglia...)<br>
                <br>
				';
...
<form enctype="multipart/form-data" class="form-horizontal" action="csv2sql_<?php echo $tabella; ?>.php" method="post">
	...
	<div class="form-group">
        <label for="xmlfile" class="control-label col-xs-2">Nome del file</label>
		<div class="col-xs-3">
        <input type="name" value="<?php echo $nomefile; ?>"  class="form-control" name="xml" id="xml" readonly>
		</div>
    </div>
	<div class="form-group">
...
//$file=$_POST['csv'];
$file = preg_replace('/[^(\x20-\x7F)]*/','', $_POST['xml']);
...
INTO TABLE '.$table.'
    CHARACTER SET "utf8"
    LINES STARTING BY "<DrctDbtTxInf>" TERMINATED BY "</DrctDbtTxInf>"
    (@DrctDbtTxInf)
SET
     mandato = ExtractValue(@DrctDbtTxInf, "//MndtId"),
     nome = ExtractValue(@DrctDbtTxInf, "//Nm"),
     iban = ExtractValue(@DrctDbtTxInf, "//IBAN"),   
     causale = ExtractValue(@DrctDbtTxInf, "//Ustrd"),             
     importo = ExtractValue(@DrctDbtTxInf, "//InstdAmt")
')or die(mysql_error());
