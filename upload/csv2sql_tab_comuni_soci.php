...
$titolo  = "Caricamento SOCI - COMUNI SOCI";
$tabella = "tab_comuni_soci";
$nomefile= "COMUSOCI.csv";
$istruzioni = ' 1. Esegui dal verdone la transazione ZW37 ed esegui la query <b>LF_SOCIIND</b> <br>
                2. Attendi il termine dell\'esecuzione <br>
                3. Lancia da verdone la <b>WW92</b> (produce il file in spool) <br>
                4. Recupera il file dallo spool AS400 (\\cartelle.mg.crtnet\mg\P) > TEMPQR.DBF e rinominalo <b>COMUSOCI.DBF</b> <br> 
                ...
                8. Rinominali con estensione CSV <b>COMUSOCI.csv</b><br>
...
// ---- Creo la tabella TAB_COMUNI_FUORIZONA solo con i records fuori zona competenza
$drop_fz=mysqli_query($cons,"DROP TABLE tab_comuni_fuorizona") or die(mysql_error());;
$crea_fz=mysqli_query($cons,
            "   CREATE TABLE tab_comuni_fuorizona as 
                SELECT c.* FROM tab_comuni_soci as c 
                         LEFT JOIN tab_comuni as t
                         ON left(c.CABcomune,5) = left(t.CAB,5)
                WHERE t.competenza <> 'S'
                GROUP BY c.cag
            "
    ) or die(mysql_error());;
//$res_fz=mysqli_fetch_array($crea_fz);
...
// Creo l'indice sul CAG
$indice_cag = mysqli_query($cons,"ALTER TABLE tab_comuni_fuorizona ADD FULLTEXT INDEX(cag)") or die(mysql_error());;
