...
<div class="col-lg-12">
	<div class="alert alert-dismissible alert-light"><h3>Statistiche Soci ChiantiBanca</h3>
	Area <?php echo $area; ?></div>
</div>
    ...
    while($tot = mysqli_fetch_array($qry_tot)){ 
        $totalesoci = $tot['qta'];
          ...
          $select_cnt = " SELECT count(*) as qta, SESSO as sessoVAL
                          ...
                          AND FILIALE_ANAGRAFICA in ".$elenco."
                          GROUP BY SESSO
                          UNION
                          SELECT count(*) as qta, 'TOTALE' as sessoVAL
                          FROM sds_sinergiareport_soci
                          WHERE DATA_ENTRATA <= NOW()
                          AND FILIALE_ANAGRAFICA in ".$elenco."
                          AND (DATA_USCITA =  '0' OR DATA_USCITA > NOW()) " ;
/*                          
    $select_cnt = " SELECT count(*) as qta, sessoVAL
                    FROM tab_soci_as37 
                    WHERE StatoVAL not in ('E','S','N')
                    AND codFil in ".$elenco."
                    GROUP BY sessoVAL" ;
...
<?php
//////////////////////////////////////////////////////////////////
// SOCIO PIE' GIOVANE e PIU' ANZIANO
//////////////////////////////////////////////////////////////////
