...
<hr>
    ...
    while($dati = mysqli_fetch_array($qry)){ 
      ...
      if    ($dati['posti_residui'] == 0) 
        ...
        if ($link == 'SI') {
          $posti_residui = '<a style="text-decoration:none;color:white;" href="eventi_gestionale.php?action=form&idevento='.$dati['idevento'].'">'.$dati['posti_residui'].'</a>';
        }
        else
          { $posti_residui = $dati['posti_residui']; }
