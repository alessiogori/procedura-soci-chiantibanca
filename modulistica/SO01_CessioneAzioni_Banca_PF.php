...
if ($_GET['motivazione'] == 'Parziale') 
    {	$titolo = "CESSIONE PARZIALE AZIONI DA SOCIO PERSONA FISICA A BANCA";
		$simbolo = "P";
    }
else 
    {	$titolo = "CESSIONE TOTALE AZIONI DA SOCIO PERSONA FISICA A BANCA";
		$simbolo = "T";
    }
...
$pdf = new Fpdi();
// get the page count
$pageCount = $pdf->setSourceFile('SO01_CessioneAzioni_Banca_PF.pdf');
...
for ($pageNo = 1; $pageNo <= $pageCount; $pageNo++) {
    ...
    if($pageNo == 1){ 
        ...
        $pdf->SetFont('Helvetica','','11');
        $pdf->SetXY(24, 73);
        //$pdf->Write(0, substr($LuogoNascita,0,18));
        $pdf->SetXY(80, 73);
        ...
        // Indirizzo Socio
        $pdf->SetXY(26, 80);
        $pdf->Write(0, $Indirizzo. ' - '.$Cap.' - '.$Localita);
        // CF e telefono
        $pdf->SetXY(32, 87);
        ...
        */
        ...
        $pdf->SetXY(20, 270);
        $pdf->Write(0, 'CAG: '.$nag.' - IDSOCIO '.$idsocio);
    }
