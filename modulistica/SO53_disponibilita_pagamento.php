        ...
        */
        ...
        if (mysqli_num_rows($querydati) <= 0) {
            ...
            while($dati=mysqli_fetch_array($querydati)){ 
                ...
                // ********************************************************
                // se la DATA USCITA non è avvalorata butto fuori
                // ********************************************************
                if ($dati['DATA_USCITA'] == 0) 
                    {
                        echo 'SOCIO NON USCITO';
                    }
                ...
                $querydati_filiale = mysqli_query($connection, $select_filiale);
                    if(mysqli_num_rows($querydati_filiale) > 0)
                        while($datifiliale = mysqli_fetch_array($querydati_filiale))
                        {
                            ...
                            $emailfiliale = $datifiliale['email_estesa'];
                        }
                    else
                    {
                        $nomefiliale = '';
                ...
                if ($dati['SOCIO_ISTITUTO'] == 9)
                {
                    switch ($dati['CTIPMOVUSCITA']) {
                      ...
                      case "  ":    // cessione a banca
                       $uscita = 'USCITO PER CESSIONE QUOTE A BANCA IL '.$dati['DATA_USCITA'];
                       $intestazione = '';
                          break;
...
$pdf = new Fpdi();
// get the page count
$pageCount = $pdf->setSourceFile('SO53_disponibilita_pagamento.pdf');
...
for ($pageNo = 1; $pageNo <= $pageCount; $pageNo++) {
    ...
    if($pageNo == 1){ 
        ...
        $pdf->SetFont('Helvetica','','11');
        $pdf->SetXY(110, 128.5);
        $pdf->Write(0, $dataUscita);
        $pdf->SetXY(19, 153);
        $pdf->Write(0, $DescFiliale.' - Tel.055 8255200 - '.$emailfiliale);        
