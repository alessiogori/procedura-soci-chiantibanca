...
// ---------------------------------------
        ...
        $selectdatiD = "     SELECT
                        ...
                        ";
        //echo $selectdati;
        $querydatiD = odbc_exec($connect, $selectdatiD);

            while($datiD = odbc_fetch_object($querydatiD)) {

                $NAG_D            = $datiD->NAG;
                $CODICE_FISCALE_D = $datiD->CODICE_FISCALE;
                $NOMINATIVO_D     = $datiD->NOMINATIVO;
                $VIA_RES_D        = $datiD->VIA_RES;
                $CAP_RES_D        = $datiD->CAP_RES;
                $DESCR_COM_RES_D  = $datiD->DESCR_COM_RES;
                $PROVINCIA_RES_D  = $datiD->PROVINCIA_RES;
                $DESCR_COM_NASC_D = $datiD->DESCR_COM_NASC;
                $PROVINCIA_NASC_D = $datiD->PROVINCIA_NASC;
                $DATA_NASCITA_D   = $datiD->DATA_NASCITA;
                $FILIALE_D        = $datiD->FILIALE_CAPOFILA;
                $NOME_FILIALE_D   = $datiD->NOME_FILIALE;
                $LUOGO_FILIALE_D  = $datiD->LUOGO_FILIALE;
                $INDIRIZZO_FILIALE_D = $datiD->INDIRIZZO_FILIALE;
                $CAP_FILIALE_D    = $datiD->CAP_FILIALE;
                $PROV_FILIALE_D   = $datiD->PROV_FILIALE;
...
$pdf = new Fpdi();
// get the page count
$pageCount = $pdf->setSourceFile('SO50_addendum_donazione.pdf');
...
for ($pageNo = 1; $pageNo <= $pageCount; $pageNo++) {
    ...
    if($pageNo == 1){ 
        ...
        $pdf->SetXY(88, 71);
        $pdf->Write(0, $LUOGO_FILIALE);
        $pdf->SetXY(32, 71);
        $pdf->Write(0, $oggi);
        // Nome Donante
        $pdf->SetFont('Helvetica','B','10');
        $pdf->SetXY(19, 77);
        $pdf->Write(0, $NOMINATIVO_D. ' (nag '.$NAG_D.'),');
        $pdf->SetFont('Helvetica','','9');
        $pdf->SetXY(19, 80);
        $pdf->Write(0, 'nato a '.$DESCR_COM_NASC_D.' ('.$PROVINCIA_NASC_D.') il '.$DATA_NASCITA_D.',');
        $pdf->SetXY(19, 83);
        $pdf->Write(0, 'residente in '.$DESCR_COM_RES_D.' ('.$PROVINCIA_RES_D.'), '.$VIA_RES_D.', C.F. '.$CODICE_FISCALE_D.',');
        $pdf->SetXY(19, 86);
        $pdf->Write(0, 'denominato di seguito, per brevita\', DONANTE;');
        // Nome Nuovo Socio
        $pdf->SetFont('Helvetica','B','10');
        $pdf->SetXY(19, 91);
        $pdf->Write(0, $NOMINATIVO. ' (nag '.$NAG.'),');
        ...
        $pdf->SetXY(19, 100);
        $pdf->Write(0, 'denominato di seguito, per brevita\', DONATARIO;');
        // Numero azioni totali
