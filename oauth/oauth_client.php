...
{/metadocument}
*/
	var $log_file_name = 'log_oauth.txt';
...
*/
	...
	var $oauth_user_agent = 'CHB-rULEz-OAuth-API ($Revision: 0.086 $)';
...
*/
	...
	Function SendAPIRequest($url, $method, $parameters, $oauth, $options, &$response)
	{
		...
		//$this->OutputDebug('MAX: Content Type: '.$request_content_type.' ');
		...
		//Forza Bearer
		if($this->access_token!="") $authorization = 'Bearer '.$this->access_token;
		...
		//echo "DEBUG XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX";
		//$post_values = $parameters;
		//var_dump($parameters);
		//var_dump($post_values);
		//var_dump($options);
		//var_dump($request_content_type);
		...
		switch($request_content_type)
		{
			...
			case 'multipart/form-data':
				if(IsSet($options['RequestBody'])){
					...
					if(IsSet($options['BodyPostValues']))
						$arguments['PostValues'] = $options['BodyPostValues'];
					else
		...
		//echo "DEBUG arguments http->SendRequest():";
		//var_dump($arguments);
	...
	Function Initialize()
	{
		...
		switch($this->server)
		{
			case 'comipa':
				...
				$this->request_token_url = 'https://services.sinergia.bcc.it/OAuthorizer/endpoint/token';
				$this->dialog_url = 'comipa';
				$this->access_token_url = 'https://services.sinergia.bcc.it/OAuthorizer/endpoint/token';
		...
		switch(intval($this->oauth_version))
		{
			case 1:
				...
				if(!IsSet($access_token['authorized']))
				{
					...
					if(!$this->SendAPIRequest($url, $method,array(	'client_id'=>$this->client_id,
